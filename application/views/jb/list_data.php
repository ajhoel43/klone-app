...
<div class="col-md-12">
	<div class="row">
		<div class="page-header">
			<h2><?php echo lang('admin_title') ?> <small>@<?php echo strtoupper(lang('label_jb')) ?></small></h2>		
		</div>
		<div class="col-md-12">
						...
						<td style="text-align:center;">
							<div class="btn-group">
								...
								<ul class="dropdown-menu">
									...
									<li><a href="<?php echo $record->kode_jb ?>" class="deletedata"><i class="fa fa-times" style="color:red;"></i> Delete</a></li>
								</ul>
							</div>
						</td>
...
$(".add-form").click(function(event){
	event.preventDefault();
	$(".form-modal").load("<?php echo base_url('jenis_bkl/add_jb') ?>");
	$(".form-modal").modal("show");
});

$("body").on("click", ".editdata",function(event){
	event.preventDefault();
	$(".form-modal").load("<?php echo base_url('jenis_bkl/upd_jb') ?>/"+$(this).attr('href'));
	$(".form-modal").modal("show");
});
...
$("body").on("click", ".deletedata",function(event){
	...
	$(".confirm-body").html('<?php echo lang("delete_q1") ?> <br><strong>Kode JB => ' + value + '</strong>');
	$(".confirm-modal").modal('show');
	$("button[name='conf_yes']").click(function(event){
		$.ajax({
			url : "<?php echo base_url('jenis_bkl/del_jb') ?>/" + value,
			type : "POST",
...
$("body").on("click", "button[name='add_data_btn']", function(event){
	event.preventDefault();
	$.ajax({
		url : "<?php echo base_url('jenis_bkl/add_jb') ?>",
		type : "POST",
...
$("body").on("click", "button[name='update_data_btn']", function(event){
	event.preventDefault();
	$.ajax({
		url : "<?php echo base_url('jenis_bkl/upd_jb') ?>/" + $(this).val(),
		type : "POST",
