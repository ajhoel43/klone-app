...
<div class="col-md-12">
	<div class="row">
		<div class="page-header">
			<h2><?php echo lang('admin_title') ?> <small>@<?php echo strtoupper(lang('label_servb')) ?></small></h2>		
		</div>
		<div class="col-md-12">
			...
			<form class="form-horizontal" role="form" name="search">
				<table class="searchform">
					<tr>
						<td width="120px">
							<label class="control-label"><?php echo lang('label_servb') ?></label>
						</td>
						<td width="400px">
					...
					<tr>
						...
						<td width="400px">
							<?php echo form_dropdown('kode_jb', $jb_dd, null, 'class="form-control"') ?>
						</td>
					</tr>
					<tr>
						<td></td>
						...
						<td style="text-align:center;">
							<div class="btn-group">
								...
								<ul class="dropdown-menu">
									<li><a href="<?php echo $record->ID_layanan ?>" class="editdata"><i class="fa fa-pencil"></i> Edit</a></li>
									<li><a href="<?php echo $record->ID_layanan ?>" class="deletedata"><i class="fa fa-times"></i> Delete</a></li>
								</ul>
							</div>
						</td>
						<td><?php echo $record->nama_jb ?></td>
						<td><?php echo $record->nama_layanan ?></td>
...
$("button[name='search']").click(function(event){
	...
	$.ajax({
		url : "<?php echo base_url('serv_bkl/search_data') ?>",
		type : "POST",
...
$(".add-form").click(function(event){
	event.preventDefault();
	$(".form-modal").load("<?php echo base_url('serv_bkl/add_servbkl') ?>");
	$(".form-modal").modal("show");
});

$("body").on("click", ".editdata",function(event){
	event.preventDefault();
	$(".form-modal").load("<?php echo base_url('serv_bkl/upd_servbkl') ?>/"+$(this).attr('href'));
	$(".form-modal").modal("show");
});
...
$("body").on("click", ".deletedata",function(event){
	...
	$(".confirm-body").html('<?php echo lang("delete_q1") ?> <br><strong>ID Layanan => ' + value + '</strong>');
	$(".confirm-modal").modal('show');
	$("button[name='conf_yes']").click(function(event){
		$.ajax({
			url : "<?php echo base_url('serv_bkl/del_servbkl') ?>/" + value,
			type : "POST",
...
$("body").on("click", "button[name='add_data_btn']", function(event){
	event.preventDefault();
	$.ajax({
		url : "<?php echo base_url('serv_bkl/add_servbkl') ?>",
		type : "POST",
...
$("body").on("click", "button[name='update_data_btn']", function(event){
	event.preventDefault();
	$.ajax({
		url : "<?php echo base_url('serv_bkl/upd_servbkl') ?>/" + $(this).val(),
		type : "POST",
